...
class Majors_set extends CI_Controller
{
  ...
  public function add_glob()
  {
    ...
    $this->session->set_flashdata('success', ' Tambah Prodi Berhasil');
  ...
  public function add($id = NULL)
  {
    ...
    $this->form_validation->set_rules('majors_name', 'Nama Prodi', 'trim|required|xss_clean');
    $this->form_validation->set_rules('majors_short_name', 'Singkatan Prodi', 'trim|required|xss_clean');
    ...
    if ($_POST and $this->form_validation->run() == TRUE) {
      ...
      $data['title'] = $data['operation'] . ' Program Studi';
  ...
  public function delete($id = NULL)
  {
    ...
    if ($_POST) {
      ...
      if (count($siswa) > 0) {
        $this->session->set_flashdata('failed', 'Data Prodi tidak dapat dihapus');
        redirect('manage/majors');
      }
      ...
      $this->session->set_flashdata('success', 'Hapus Prodi berhasil');
      redirect('manage/majors');
    } elseif (!$_POST) {
