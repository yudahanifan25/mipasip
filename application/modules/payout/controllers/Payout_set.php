...
class Payout_set extends CI_Controller
{
  ...
  public function index($offset = NULL, $id = NULL)
  {
    ...
    $data['bulan'] = $this->Bulan_model->get(array('payment_id' => $id, 'student_id' => $siswa['student_id']));
  ...
  function printBill()
  {
   $this->load->helper(array('dompdf'));
    ...
    $data['s_bb'] = $this->Bebas_pay_model->get($cashback);
     $data['s_tag'] = $this->Bebas_pay_model->get($cashback);
    ...
    foreach ($data['s_bb'] as $row) {
     ...
     $data['sumtag'] = 0;
    foreach ($data['s_tag'] as $row) {
      $data['sumtag'] += $row['bebas_pay_bill'] - $row['bebas_pay_bill'];
    }
            $data['total_bebas'] = 0;
  ...
  function printKartu()
  {
    ...
    $html = $this->load->view('payout/payout_kartu_bayar', $data, true);
    $data = pdf_create($html, $siswa['student_nis'] . ' - ' . $siswa['student_full_name'], TRUE, 'A5', 'landscape');
  }
  ...
  function printBayar()
  {
    ...
    $html = $this->load->view('payout/payout_kartu', $data, true);
    $data = pdf_create($html, $siswa['student_nis'] . ' - ' . $siswa['student_full_name'], TRUE, 'A5', 'landscape');
  }
  ...
  function cetakBukti()
  {
    ...
    $data['bulan'] = $this->Bulan_model->get($param);
    $data['bebas'] = $this->Bebas_model->get($pay);
    $data['bebas_bill'] = $this->Bebas_model->get($pay);
    $data['free'] = $this->Bebas_pay_model->get($param);
    $data['s_bl'] = $this->Bulan_model->get_total($cashback);
    $data['s_bb'] = $this->Bebas_pay_model->get($cashback);
  ...
  function pay($payment_id = NULL, $student_id = NULL, $id = NULL)
  {
    ...
    if ($activated['setting_value'] == 'Y') {
      ...
      $namePay = $student['pos_name'] . ' - T.P ' . $student['period_start'] . '/' . $student['period_end'];
    ...
    if ($this->input->is_ajax_request()) {
      ...
      // redirect('manage/payout?n=' . $student['period_period_id'] . '&r=' . $student['student_nis']);
      redirect('manage/payout/payout_bayar/' . $student['payment_payment_id'] . '/' . $student_id);
    }
  }
  ...
  function update_pay_desc()
  {
    ...
    if ($this->input->is_ajax_request()) {
      ...
      $this->session->set_flashdata('success', 'Update Keterangan Berhasil');
      // redirect('manage/payout?n=' . $period_id . '&r=' . $student_nis);
      redirect('manage/payout/payout_bayar/' . $payment_id . '/' . $student_id);
    }
  }
