...
<body style="color: #fff;">
	...
	<table width="100%" border="0" style="font-size: 12px; margin-top: -10px">
		<tr>
			<td width="150">Nomor Induk Sekolah</td>
			<td width="5">:</td>
			<?php foreach ($siswa as $row) : ?>
	...
	<br>
	<?php if ($f['n'] and $f['r'] != NULL and $f['d'] != NULL) { ?>
		<table width="100%" style="font-size: 12px; margin-top: -10px;">
			...
			?>
				<tr>
					...
					<td style="white-space: nowrap; padding:0 10px;"><?php echo ($row['bulan_status'] == 0) ? $namePay . ' - ' . '......................' : $namePay . ' - ' . (($f['d'] == $row['bulan_date_pay']) ? '<b style="color:#000">' . $row['month_name'] . ' ' . $mont . '</b>' : '') ?></td>
					<td style="padding:0 10px; white-space: nowrap;"><?php echo ($row['bulan_status'] == 0) ? 'Rp. ......................' : (($f['d'] == $row['bulan_date_pay']) ? '<b style="color:#fff">Rp. </b><b style="color:#000">' . number_format($row['bulan_bill'], 0, ',', '.') . '</b>' : '') ?></td>
	...
	<?php } else redirect('manage/payout?n=' . $f['n'] . '&r=' . $f['r'] . $f['d'])  ?>
